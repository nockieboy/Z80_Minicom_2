...
;
; 29/03/17 - Updated to 4.7b1 after line buffer was moved further up RAM and increased from 72 to 138 characters
; 18/04/17 - Updated to 4.7b3; testing addition of new command CLRMEM
...
ESC     		.EQU    	1BH             	; Escape
DEL     		.EQU    	7FH             	; Delete
LINELEN	.EQU		8AH				; Maximum line length

; BASIC WORK SPACE LOCATIONS

WRKSPC  .EQU    BASCLD+20D0H      ; BASIC Work space (RAM_BOTTOM is defined in minicomp.asm)
...
PROGST  .EQU    WRKSPC+186H     ; Start of program text area
STLOOK  .EQU    WRKSPC+1EAH     ; Start of memory test

; BASIC ERROR CODE VALUES

NF      .EQU    00H             ; NEXT without FOR
SN      .EQU    11H             ; Syntax error
RG      .EQU    18H             ; RETURN without GOSUB
OD      .EQU    1BH             ; Out of DATA
FC      .EQU    1EH             ; Function call error
OV      .EQU    21H             ; Overflow
OM      .EQU    24H             ; Out of memory
UL      .EQU    27H             ; Undefined line number
BS      .EQU    2AH             ; Bad subscript
DD      .EQU    2DH             ; Re-DIMensioned array
DZ      .EQU    30H             ; Division by zero (/0)
ID      .EQU    33H             ; Illegal direct
TM      .EQU    36H             ; Type miss-match
OS      .EQU    39H             ; Out of string space
LS      .EQU    3CH             ; String too long
ST      .EQU    3FH             ; String formula too complex
CN      .EQU    42H             ; Can't CONTinue
UF      .EQU    45H             ; UnDEFined FN function
MO      .EQU    48H             ; Missing operand
HX      .EQU    4BH             ; HEX error
BN      .EQU    4EH             ; BIN error
...
B_INIT: LD      DE,INITAB       ; Initialise workspace
...
COPY:   LD      A,(DE)          ; Get source
        ...
        LD      (BUFFER+LINELEN+1),A ; Mark end of buffer
...
SIGNON: .BYTE   "Z80 BASIC Ver 4.7b5",CR,LF
        .BYTE   "Copyright ",40,"C",41
        .BYTE   " 1978 Microsoft",CR,LF,0,0

CLRMSG:	.BYTE	"Memory cleared",CR,LF,0,0
TSTMSG:	.BYTE	"This is a test message!",CR,LF,0
...
WORDTB: .WORD   PEND
        ...
        .WORD   LIST
        .WORD   CLEAR
        .WORD   TEST			; TEST token is directed to TEST VAR
        .WORD   CLRMEM			; CLRMEM token is directed to CLRMEM VAR
        .WORD   NEW
...
ERRORS: .BYTE   "NEXT without FOR",0    ; NEXT without FOR
        .BYTE   "Syntax",0          ; Syntax error
        .BYTE   "RG",0          ; RETURN without GOSUB
        .BYTE   "OD",0          ; Out of DATA
        .BYTE   "FC",0          ; Illegal function call
        .BYTE   "OV",0          ; Overflow error
        .BYTE   "OM",0          ; Out of memory
        .BYTE   "UL",0          ; Undefined line
        .BYTE   "BS",0          ; Bad subscript
        .BYTE   "DD",0          ; Re-DIMensioned array
        .BYTE   "/0",0          ; Division by zero
        .BYTE   "ID",0          ; Illegal direct
        .BYTE   "TM",0          ; Type mis-match
        .BYTE   "OS",0          ; Out of string space
        .BYTE   "LS",0          ; String too long
        .BYTE   "ST",0          ; String formula too complex
        .BYTE   "CN",0          ; Can't CONTinue
        .BYTE   "UF",0          ; Undefined FN function
        .BYTE   "MO",0          ; Missing operand
        .BYTE   "HX",0          ; HEX error
        .BYTE   "BN",0          ; BIN error
...
SNERR:  LD      DE,SN            ; ?SN Error
        .BYTE   01H             ; Skip "LD E,DZ"
DZERR:  LD      DE,DZ            ; ?/0 Error
        .BYTE   01H             ; Skip "LD E,NF"
NFERR:  LD      DE,NF            ; ?NF Error
        .BYTE   01H             ; Skip "LD E,DD"
DDERR:  LD      DE,DD            ; ?DD Error
        .BYTE   01H             ; Skip "LD E,UF"
UFERR:  LD      DE,UF            ; ?UF Error
        .BYTE   01H             ; Skip "LD E,OV
OVERR:  LD      DE,OV            ; ?OV Error
        .BYTE   01H             ; Skip "LD E,TM"
TMERR:  LD      DE,TM            ; ?TM Error
...
PRTERR: 
		...
		CP		EOS              	; Is it $00 ?
		JP		NZ,PRTERR		; Not $00, loop and print
...
PUTCTL: LD      A,B             ; Get number of bytes in buffer
        CP      LINELEN+1    ; Test for line overflow
...
TEST:	LD		HL,TSTMSG		; Load text message
		CALL	PRS				; Print it
		JP		PRNTOK			; Return to direct mode
		
; CLRMEM - Clears the memory (sets to 0) from start address to #FFFF
CLRMEM: CALL	DEINT			; Get start memory address into DE
CLRP:	LD		HL,LSTRAM		; Load top RAM address into HL
		LD		A,00H			; Set zero value to set in memory
		LD		(DE),A			; Set the memory address to zero
		INC		DE				; Next memory location
		OR		A				; Reset C flag
		SBC		HL,DE			; Compare HL with DE to see if we've
		ADD		HL,DE			; reached the end of free RAM
		JP		NZ,CLRP
		LD		(DE),A			; Clear the final memory location
		LD		HL,CLRMSG		; Load completion message
		CALL	PRS				; Print it
		JP		PRNTOK			; Return to direct mode		
